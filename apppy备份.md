...
(function(){
    try {
        var __orig_console_log = console.log;
        console.log = function() {
            try {
                send({__frida_console: true, args: Array.prototype.slice.call(arguments)});
            } catch(e) {}
            try { __orig_console_log.apply(console, arguments); } catch(e) {}
        };
        var __orig_console_error = console.error;
        console.error = function() {
            try { send({__frida_console: true, args: Array.prototype.slice.call(arguments), level: 'error'}); } catch(e) {}
            try { __orig_console_error.apply(console, arguments); } catch(e) {}
        };
    } catch(e) {}
})();
"""
def wrap_user_script_wait_java(user_js: str, max_attempts: int = 150, delay_ms: int = 100) -> str:
    """
    Wrap user_js so it will only call Java.perform(...) when Java is available.
    Returns a JS string (no console wrapper).
    ...
    # Keep a simple marker __USER_CODE__ and then replace it with user_js.
    # We avoid f-string braces conflicts by using replace.
    wrapper = (
        "(function(){\n"
        "  var __frida_user_script = function(){\n"
        "__USER_CODE__\n"
        "  };\n\n"
        "  function __run_user(){\n"
        "    try{\n"
        ...
        "        Java.perform(function(){\n"
        "          try{ __frida_user_script(); }catch(e){ console.error('[USER ERR]', e); }\n"
        "        });\n"
        "        return true;\n"
        "      }\n"
        "    }catch(e){}\n"
        ...
        "  if (!__run_user()){\n"
        "    var __attempts = 0;\n"
        f"    var __max = {int(max_attempts)};\n"
        f"    var __delay = {int(delay_ms)};\n"
        "    var __interval = setInterval(function(){\n"
        "      try{ __attempts++; }catch(e){}\n"
        "      if (__run_user() || __attempts >= __max){\n"
        "        try{ clearInterval(__interval); }catch(e){}\n"
        "      }\n"
        "    }, __delay);\n"
        "  }\n"
        "})();\n"
    )
    # insert the user's code (no surrounding function braces added here because wrapper already puts it inside a function)
    return wrapper.replace("__USER_CODE__", user_js)
...
def _start_frida_via_subprocess(cmd, script_path):
    """
    Spawn frida CLI and create background tasks to read stdout/stderr and forward to socketio.
    """
    try:
        p = subprocess.Popen(cmd, stdout=subprocess.PIPE, stderr=subprocess.PIPE)
        pid = p.pid
        logging.info("Started frida subprocess pid=%s cmd=%s", pid, " ".join(cmd))
        socketio.emit("frida_started", {"pid": pid, "script": script_path, "spawn": ("-f" in cmd)})
        ...
        # start background tasks using socketio helper (threading mode)
        socketio.start_background_task(reader_thread, p.stdout, "")
        socketio.start_background_task(reader_thread, p.stderr, "[ERR] ")

        return {"status":"ok", "pid": pid, "script": script_path, "spawn": ("-f" in cmd)}
    except Exception as e:
...
def _start_frida_via_api_sync(target_app, script_text, use_spawn):
    ...
    This function blocks until script.load() completes or raises.
    NOTE: this function expects script_text WITHOUT the CONSOLE_WRAPPER
    because it will prepend CONSOLE_WRAPPER itself.
    """
...
# ------------------ Auto-wrap user script to wait for Java.runtime ------------------
def wrap_user__script_wait_java(user_script: str, max_attempts: int = 150, interval_ms: int = 100) -> str:
    """
    Wrap user_script so that it will wait for Java runtime to be available,
    then execute the user's code inside Java.perform.
    Returns wrapped script WITHOUT the CONSOLE_WRAPPER (so caller can decide).
    """
    # sanitize user's script: put it inside a function to avoid polluting wrapper scope
    # We won't try to parse JS; simply inject user code inside function body.
    wrapper = f"""
(function(){{
    // user code as function
...
{user_script}
    ...
    function _tryExec() {{
            ...
            if (typeof Java !== 'undefined' && typeof Java.perform === 'function') {{
                Java.perform(function() {{
                    try {{
                        ...
                        try {{ send({{"__frida_console": true, "args":[ "[auto-wrap] user_main error:", String(e) ], "level":"error"}}); }} catch(e2){{}}
                    }}
                }});
                return;
            }}
        ...
        if (attempts < {max_attempts}) {{
            setTimeout(_tryExec, {interval_ms});
        }} else {{
            // give up waiting for Java - attempt to run user code in global context (may fail)
            try {{
                __user_main();
            }} catch(e) {{
                try {{ send({{"__frida_console": true, "args":[ "[auto-wrap] final exec err:", String(e) ], "level":"error"}}); }} catch(e2){{}}
            }}
        }}
    }}
...
def run_frida_custom():
    ...
    try:
        ...
        if not target_app:
            return jsonify({"status":"error", "error":"missing_target_app"}), 400
        if not script_text or not isinstance(script_text, str):
            return jsonify({"status":"error", "error":"missing_script"}), 400
        if len(script_text) > 200_000:
            return jsonify({"status":"error", "error":"script_too_large"}), 400

        # Wrap user script to wait for Java (no console wrapper in this string)
        wrapped_no_console = wrap_user_script_wait_java(script_text)

        # Persist a CLI-ready script file (console wrapper + wait-wrapper)
        script_name = f"frida_custom_{int(time.time())}_{uuid.uuid4().hex[:8]}.js"
        script_path = os.path.join(FRIDA_DIR, script_name)
        try:
            with open(script_path, "w", encoding="utf-8") as f:
                # CLI needs console wrapper in file so stdout/stderr prints are captured by frida CLI
                f.write(CONSOLE_WRAPPER + "\n" + wrapped_no_console)
        except Exception as e:
            logging.error("write custom script failed: %s\n%s", e, traceback.format_exc())
            return jsonify({"status":"error", "error":"write_failed", "detail": str(e)}), 500

        # Preferred: frida-python
        if FRIDA_PY_AVAILABLE:
            try:
                # detect running process for spawn decision
                try:
                    ps = subprocess.run(["frida-ps", "-Uai"], capture_output=True, text=True, timeout=6)
                    p_stdout = ps.stdout or ""
                    proc_found = any((target_app in ln) for ln in p_stdout.splitlines())
                ...
                # Use API helper (it will prepend CONSOLE_WRAPPER internally), pass wrapped_no_console
                res = _start_frida_via_api_sync(target_app, wrapped_no_console, use_spawn)
                if not isinstance(res, dict):
                    res = {"status":"error", "detail": str(res)}
                status_code = 200 if res.get("status") == "ok" else 500
                logging.info("run_frida_custom(api) target=%s spawn=%s result=%s", target_app, use_spawn, res)
                return jsonify(res), status_code
            except Exception as e:
                logging.error("run_frida_custom frida-py path exception: %s\n%s", e, traceback.format_exc())
                # fallthrough to CLI fallback

        # CLI fallback: make sure frida CLI exists
        try:
            check = subprocess.run(["frida","--version"], capture_output=True, text=True, timeout=5)
            if check.returncode != 0:
                return jsonify({"status":"error", "error":"frida_cli_unavailable", "detail": check.stderr or check.stdout}), 500
        except Exception as e:
            return jsonify({"status":"error", "error":"frida_cli_check_failed", "detail": str(e)}), 500

        # decide spawn vs attach for CLI
        try:
            ps = subprocess.run(["frida-ps", "-Uai"], capture_output=True, text=True, timeout=8)
            p_stdout = ps.stdout or ""
            proc_found = any((target_app in ln) for ln in p_stdout.splitlines())
        ...
        if not isinstance(res, dict):
            res = {"status":"error", "detail": str(res)}
        status_code = 200 if res.get("status") == "ok" else 500
        logging.info("run_frida_custom(cli) target=%s spawn=%s cmd=%s result=%s", target_app, use_spawn, " ".join(cmd), res)
        return jsonify(res), status_code

    except Exception as e:
        logging.error("run_frida_custom top-level exception: %s\n%s", e, traceback.format_exc())
        return jsonify({"status":"error", "error":"internal_exception", "detail": str(e)}), 500
...
def run_frida_custom():
    ...
    # 保存脚本文件供 CLI 调用（包含 console wrapper + 用户脚本）
    script_name = f"frida_custom_{int(time.time())}_{uuid.uuid4().hex[:8]}.js"
    script_path = os.path.join(FRIDA_DIR, script_name)
    try:
        with open(script_path, "w", encoding="utf-8") as f:
    ...
    if FRIDA_PY_AVAILABLE:
        ...
        # call API helper with original script (it will prepend CONSOLE_WRAPPER internally in the API helper)
        res = _start_frida_via_api_sync(target_app, script_to_run, use_spawn)
